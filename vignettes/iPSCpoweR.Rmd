...
We can get the mean number of false positives, as well as (assuming that `addDE` was enabled) true positives and FDR:
```{r}
cbind(
...
```{r sensMat, fig.width=3.5, fig.height=3.5}
...
Of note, all four methods are very slow, and according to our study less accurate than using `limma::duplicateCorrelation` as discussed above.
